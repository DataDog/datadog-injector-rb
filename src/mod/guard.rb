...
class << self
  def call(status)
    ...
    if !status[:bundler][:bundled]
      result << { :name => 'bundler.in_bundle', :reason => 'bundler.unbundled' }
    end
    ...
    if status[:bundler][:path]
      result << { :name => 'bundler.path', :reason => 'bundler.path' }
    end
    ...
    if status[:dependency][:datadog]
      result << { :name => 'dependency.datadog', :reason => 'datadog.manual' }
    end

  # if !status[:dependency][:rails]
  #   result << { :name => 'dependency.rails', :reason => 'framework.unsupported' }
  # end
