...
assert('payload did not become inheritable after validation') do
...
pid = Process.spawn(
  ...
  RbConfig.ruby,
  '-e',
  "begin; #{script}; exit 0; rescue Errno::EBADF; exit 1; end",
  { :close_others => false }
)
_, status = Process.waitpid2(pid)
assert('payload descriptor did not follow the process bundle') { status.success? }
...
args = [
  RbConfig.ruby,
  '-e',
  'begin; IO.new(ENV.fetch("FD").to_i).fcntl(1034); exit 1; rescue Errno::EBADF; exit 0; end',
  { :close_others => false },
]
consumer.close_exec!(args)
pid = Process.spawn({ 'FD' => fd.to_s }, *args)
_, status = Process.waitpid2(pid)
assert('payload descriptor survived an explicit close mapping') { status.success? }
